            ...
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        // Update this path to use a relative folder path for better management
                        string folderPath = Path.Combine(Application.StartupPath, "Books_Directory");
                        string filePath = Path.Combine(folderPath, addBooks_bookTitle.Text + addBooks_author.Text.Trim() + ".jpg");
                        ...
                        File.Copy(addBooks_picture.ImageLocation, filePath, true); // Copy the image to the new folder

                        string insertData = "INSERT INTO books " +
                            "(book_title, author, published_date, status, image, date_delete) " +
                            "VALUES(@bookTitle, @author, @published_date, @status, @image, @dateDelete)";
                        ...
                        using (SqlCommand cmd = new SqlCommand(insertData, connect))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@image", filePath); // Use the new file path
                            cmd.Parameters.AddWithValue("@dateDelete", today); // Set the current date
